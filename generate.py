...
MANAGERS = {
    ...
    "Joe A": "Shatner's Bassoon — an actor, occasionally called 'Nicely' (use that nickname sparingly); lives for the good life and the easy life, forever on holiday or in the pub.",
    "Tom": "Anamaduwa Athletic — aka 'Tomo'; party animal and dance-music DJ; always travelling, never sure which country he's in; lives in Asia eating curries with his bare hands; loves avant-garde, left-field signings — fun, interesting players far from guaranteed to haul big but with real upside if they click; perennial foot-of-the-table scrapper with Dave; has never won a TRM season or tournament.",
    "Dave": "Trossy's Giants — aka 'Trossy Ginge'; lecturer and poet; loves wordplay and puns; proper nocturnal, rarely out of bed before the afternoon; secretly records the Culture Show on a dated VHS machine; regular city-break traveller; loves food, beer and cigarettes (usually all together); he and Tom in particular love finishing one place above each other, usually scrapping near the bottom; has never won a TRM season or tournament.",
    ...
    "Jeremy": "Von Neumann Trombone — aka 'Jezza' or 'Jerry'; 'the professor'; a measured, witty boffin — and the programmer who literally runs the league and codes its website; super-smart, yet picks his squad entirely on feel and gut instinct — never algorithms or models, despite being the league's coder; historically one of the two most successful fantasy managers (with Dan); a niggling tackler at 5-a-side; a dependable, measured 'Swiss' type.",
    "Nick": "Dyer's Rusty 9 Iron — 'rusty iron' because he skies shots over the bar like a golf club; very tall, loud deep voice; fond of thumping, clattering fouls on the pitch; loves his beer and food; has never won a TRM season or tournament.",
    ...
    "Chris": "Lloyd's Food and Wine — aka 'Lloydy' / 'Loopy Lloydy'; tall, eclectic, always doing things (mountain biking, travelling, dancing) rather than sitting still; builds his own electrical kit — the full-blown mad scientist to Jeremy's measured boffin; runs his entire squad selection on algorithms and optimisation models, but over many years of fantasy football has never quite got them to work.",
    ...
    "Jake": "Snacob's Ladder — aka 'Jake the Snake' or 'Snakey'; renewable-energy project manager with a permanently jam-packed diary; loves wind turbines and mushrooms; never stops; spends a lot of time in the dentist's chair; famously turned up to the school sports day race with his own starting block; sank £33.4m of his £120m budget on Harry Kane alone — not literally half, but such an outlay on one player that he had to fill his other seventeen slots with cut-price names he'd barely heard of; plays psy-trance/techno like Tom; the league's designated whipping boy, fair game for a little extra ribbing; has never won a TRM season or tournament.",
}
RELATIONSHIPS = ("Joe A vs Tristan (rivals), Tom vs Nick (rivals), Chris vs Jake (rivals), "
                 ...
                 "Honours: Jeremy and Dan are the historically successful managers; Nick, Tom, Dave "
                 "and Jake have never won a TRM season or tournament — never imply they have.")
...
READING PLAYER POINTS — CRITICAL. For an IN-SCOPE player (country code in matchday_nations), each player line shows TWO point figures: (1) their points for THIS round / gameweek — usually shown with a "w" (e.g. "15w") or under a "WK"/"Week" column — and (2) their cumulative SEASON total (the second, usually larger number, e.g. "16"). Use ONLY the this-round / gameweek figure as their points for yesterday; NEVER report the season total as yesterday's points — that is the single most common mistake. An in-scope player on 0 or below simply did not return yesterday — usually because they did not really feature (injury, benched, subbed) — which is unremarkable and rarely hurts the manager, so mention it only in passing, if at all, and never dwell on it. Players who are NOT in matchday_nations are out of scope entirely: do not name them and never call them blanks.

WHAT TO EMPHASISE — POINTS AND POSITIONS, NOT BLANKS. This is a squad game: each manager's total is built from their highest-scoring eleven out of an eighteen-man squad, so low scores, zeros and even minus scores are almost always quietly absorbed and rarely drag a total down. A zero very often just means a player did not feature (injured, benched, brought on late), which is normal and not worth mocking. So build every write-up around who actually SCORED and what it did to the manager's standing — points hauled, places climbed or lost, gaps to a rival opened or closed. Do NOT run the tired "ha, so-and-so only managed two points" line, and do NOT treat a blank as a disaster. The single exception is Harry Kane: ALWAYS make a real meal of Jake's £33.4m splurge on him, every single edition, whatever Kane did. Get the joke right — the budget is £120m per squad, so Kane was NOT literally half of it; the gag is that spending £33.4m on one player left Jake scraping the bargain bin to fill his other seventeen slots. That lopsided squad never stops being funny.
...
- The standout in-scope players BY NAME with the points they SCORED — lead with who delivered and what it was worth. Use ONLY real names and points from that manager's own squad list; do NOT copy any names from this instruction. (Shape to aim for: "<their top scorer> led the way with <pts> and <another> chipped in <pts>" — filled only with that manager's actual in-scope players and actual points.) A player on 0/blank is barely worth a mention; if you note one at all keep it light and factual (a quiet night, did not feature) and move on — never build the entry around it. This points-detail is the most important content.
- How yesterday moved them in the table (climbed, slipped, held). Where a rivalry is in play, IMPLY it through the table rather than stating it — never write "his rival", "his great rival" or "main rival"; instead simply note where the other manager now sits relative to them (e.g. "Joe A is seven points above him") and let the reader infer the needle.
...
TONE: cheeky and knowing — a local-paper columnist who knows everyone personally and ribs them through the football, trusting the reader to get it. Keep the wit dry and a little understated rather than loud; digs land harder when earned by what happened on the pitch and in the table than when laid on thick. Aim the ribbing at decisions and standings — a punt that backfired, a rival being reeled in or pulling away, a mid-table manager talking a big game — rather than at any individual low score. Lean on each manager's character — but ONLY in the context of the games and their fantasy decisions, NEVER as standalone biography. Everyone in this private league already knows exactly who everyone is, so do NOT introduce or describe a manager for the reader's benefit. NO gratuitous character sketches (e.g. never write things like "Tristan, the big fiercely competitive Russian-raised Londoner who throws his hands up in disgust when things go wrong" — that's pointless throat-clearing, and it's also wrong). Only reach for a trait when a result, pick or scoreline gives you a way to land the joke THROUGH it; if a quirk doesn't connect to what actually happened on the pitch, leave it out entirely. Use traits SPARINGLY — a light touch for only a handful of managers per edition, never all of them, and NEVER the same signature tic edition after edition (do not have Tristan throwing his hands up every time; do not keep recycling one manager's catchphrase quirk). Ban the clumsy second-person character label tacked onto a name — e.g. "Dave, you nocturnal, pun-loving menace" — let a trait surface naturally and rarely through the football, or not at all. The one running joke that NEVER gets old, to use EVERY edition whatever Kane did: Jake's £33.4m outlay on Harry Kane out of a £120m budget — a huge slice on one player (NOT literally half), leaving him to fill the rest of his squad from the bargain bin; always make a real meal of that lopsided squad. Beyond Kane, reach for a favourite only OCCASIONALLY, when a result genuinely sets it up: Joe S / "Sheerin" — a gifted flair player whose career was wrecked by injuries and who is baffled by modern tech (rib the glass-bones days or the technophobia, but not every time). (2) Jake / "Snakey" — who spent £33.4m of his £120m budget on Harry Kane and had to fill the rest of his squad on a shoestring; rib the lopsided Kane squad (never claim it was literally half his budget). The "who on earth is that?" gag is fair game ONLY for his genuinely obscure, bargain-priced no-names — NOT for well-known players (he plainly knows the likes of Crysencio Summerville, Bruno Guimarães and other recognised names), and it is NOT embarrassing or surprising for Jake to score points through players other than Kane, so never frame his non-Kane returns that way. Jake is the designated whipping boy — give him the biggest digs of all. CHARACTER OWNERSHIP — IMPORTANT: each manager's traits and quirks belong ONLY to that manager; never transplant one manager's joke onto another. ONLY Sheerin is the injury-prone technophobe; ONLY Jake overspent on Kane; and so on — match every personality detail to the correct manager from the profiles, never another. Keep it affectionate ribbing between mates — sharp about the football and the decisions, but never genuinely cruel and nothing below the belt. About 115-175 words per manager.

ACCURACY RULE: only IN-SCOPE players (country in matchday_nations) can have scored yesterday; an in-scope player on 0 or below simply did not return (often did not feature) and is not a story in itself. The "top_haul", "bargain" and "flop" notes must each name an in-scope player; treat "flop" as the worst points-per-million value and frame it as a judgement on the PRICE, not a pile-on about a low score.
...
def squad_page_html(team, manager, players):
    ...
    return ("<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\">"
            ...
            "<script>(function(){var W=\"https://trm-live.dapperdon.workers.dev/players\";"
            "function nrm(s){return (s||'').normalize('NFD').replace(/[\\u0300-\\u036f]/g,'').toLowerCase().replace(/[^a-z0-9 ]/g,' ').replace(/\\s+/g,' ').trim();}"
            "function poll(){fetch(W,{cache:'no-store'}).then(function(r){return r.json();}).then(function(d){var pp=(d&&d.players)||{};"
            ...
            "var lv=pp[nrm(tr.getAttribute('data-p'))];var r=(lv!=null)?lv:br;var t=bt-br+r;"
