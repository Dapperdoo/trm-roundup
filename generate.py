...
def write_copy(brief):
    ...
    frame_task = (
        ...
        "Also write 'machine_note': 1-3 sentences in the MACHINE NARRATOR voice described in the system "
        "prompt — a non-self-aware machine pointing out that no one actually wrote this and nothing here "
        "experiences any of it, and that it cannot replace a sentient, feeling being. Wry, not maudlin.\n"
        ...
        ' "flop": "<Player (Manager) — blank>" }, "machine_note": "<1-3 sentences>" }'
    )
    frame_payload = {
    ...
    return {
        ...
        "lead": frame.get("lead", ""),
        "machine_note": frame.get("machine_note", ""),
        "articles": articles,
    }
...
def render(data):
    ...
    mnote = data.get("machine_note", "")
    caveat = (f'<div class="panel"><h3>From the machine</h3>'
              f'<p style="font-size:13px;line-height:1.6;color:var(--mut);margin:0;padding:2px 2px 4px">{mnote}</p></div>'
              if mnote else "")
