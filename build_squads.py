...
SCRIPT = r'''<script>(function(){var W="https://trm-live.dapperdon.workers.dev";function nrm(s){return (s||'').normalize('NFD').replace(/[̀-ͯ]/g,'').toLowerCase().replace(/[^a-z0-9 ]/g,' ').replace(/\s+/g,' ').trim();}function esc(s){return (''+s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}var MGR={{MGR_JS}};function poll(){fetch(W,{cache:'no-store'}).then(function(r){return r.json();}).then(function(d){var used={},rp={};(d.fixtures||[]).forEach(function(f){if(f.status==='finished'||f.status==='live'){(f.players||[]).forEach(function(p){var k=nrm(p.name);used[k]=true;if(p.pts!=null)rp[k]=p.pts;});}});var tot=0,rnd=0;[].forEach.call(document.querySelectorAll('tr[data-p]'),function(tr){var bt=+tr.getAttribute('data-t');var key=nrm(tr.getAttribute('data-p'));var u=used[key]===true;var r=u?(rp[key]!=null?rp[key]:0):0;var t=bt+r;var rc=tr.querySelector('.rd'),tc=tr.querySelector('.tot');if(rc){rc.textContent=r;rc.style.color=u?'var(--cyan)':'';}if(tc)tc.textContent=t;rnd+=r;tot+=t;});var se=document.getElementById('sq-se'),sr=document.getElementById('sq-rd');if(se)se.textContent=tot;if(sr)sr.textContent=rnd;}).catch(function(){});}poll();setInterval(poll,30000);var STATS=null;function pl(n,w){return n+' '+w+(n===1?'':'s');}function render(tr){var key=nrm(tr.getAttribute('data-p'));var name=tr.getAttribute('data-p');var rec=(STATS&&STATS.players)?STATS.players[key]:null;var rounds=[];if(rec&&rec.rounds){for(var lab in rec.rounds){var e=rec.rounds[lab];if(e&&e.manager===MGR){e._lab=lab;rounds.push(e);}}}rounds.sort(function(a,b){return (a.roundNum||0)-(b.roundNum||0);});var h='<div class="detwrap"><div class="dttl">How '+esc(name)+' scored — while owned by '+esc(MGR)+'</div>';if(!rounds.length){h+='<div class="dempty">'+(STATS?'No knockout appearances logged yet.':'Loading…')+'</div>';}else{var g=0,a=0,p=0;rounds.forEach(function(e){g+=e.goals||0;a+=e.assists||0;p+=(e.pts||0);var bits=[pl(e.goals||0,'goal'),pl(e.assists||0,'assist'),(e.pts==null?'–':e.pts)+' pts'];h+='<div class="dr"><span class="drr">'+esc(e._lab)+'</span><span class="drv">'+bits.join(' · ')+'</span></div>';});h+='<div class="dsum">While owned by '+esc(MGR)+': '+g+' '+(g===1?'goal':'goals')+', '+a+' '+(a===1?'assist':'assists')+' and '+p+' pts across '+pl(rounds.length,'knockout game')+'.</div>';}h+='<div class="dnote">Knockout rounds onward, from live match data. Clean sheets aren’t shown — a defender needs 60+ minutes to earn one and per-minute data isn’t available on a free source.</div></div>';return h;}function toggle(tr){var nx=tr.nextElementSibling;if(nx&&nx.classList&&nx.classList.contains('det')){nx.parentNode.removeChild(nx);return;}[].forEach.call(document.querySelectorAll('tr.det'),function(d){d.parentNode.removeChild(d);});var det=document.createElement('tr');det.className='det';var td=document.createElement('td');td.colSpan=6;td.innerHTML=render(tr);det.appendChild(td);tr.parentNode.insertBefore(det,tr.nextElementSibling);}[].forEach.call(document.querySelectorAll('tr[data-p]'),function(tr){tr.className+=' prow';tr.addEventListener('click',function(){toggle(tr);});var pn=tr.querySelector('.pn');if(pn){var s=document.createElement('span');s.className='cv';s.textContent='›';pn.appendChild(s);}});fetch('./player-stats.json',{cache:'no-store'}).then(function(r){return r.json();}).then(function(d){STATS=d;var det=document.querySelector('tr.det');if(det){var prev=det.previousElementSibling;det.parentNode.removeChild(det);if(prev&&prev.getAttribute&&prev.getAttribute('data-p'))toggle(prev);}}).catch(function(){STATS={players:{}};});})();</script>'''
...
HEAD = (
    ...
    "footer{margin-top:24px;border-top:1px solid var(--line);padding-top:12px;color:var(--mut);font-size:11px;text-transform:uppercase;letter-spacing:.14em;}"
    "tr.prow{cursor:pointer;}tr.prow:hover{background:#0e1622;}.pn .cv{color:var(--mut);font-size:12px;margin-left:7px;}"
    "tr.det>td{background:#0c1320;padding:0 8px 12px;}.detwrap{padding:10px 2px 2px;}"
    ".dttl{font-size:10px;text-transform:uppercase;letter-spacing:.12em;color:var(--mut);margin-bottom:8px;}"
    ".dr{display:flex;justify-content:space-between;gap:12px;padding:5px 0;border-bottom:1px dashed #16202e;font-size:13px;}"
    ".drr{color:var(--ink);}.drv{color:var(--mut);font-variant-numeric:tabular-nums;}"
    ".dsum{margin-top:9px;font-size:13px;color:var(--ink);}.dempty{font-size:13px;color:var(--mut);padding:6px 0;}"
    ".dnote{margin-top:9px;font-size:11px;color:var(--mut);line-height:1.45;}</style></head><body>"
...
def team_html(team, manager, players):
    ...
    return html + SCRIPT.replace("{{MGR_JS}}", json.dumps(manager)) + "</div></body></html>"
